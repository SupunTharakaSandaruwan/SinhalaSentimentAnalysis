...
def get_adjectives(response_text):
    ...
    f1 = codecs.open("D:\\Education\\Z\\Filtered\\PLAIN_SENTENCES_WITH_CON_ADJ_" + str(part_no) + ".TXT", "a", encoding='utf-8')
        ...
        if re.findall('.{1,}_JJ', word):
            ...
            if re.findall('.{1,}_CC', pre_word):
                ...
                if re.findall('.{1,}_JJ', pre_pre_word):
                    ...
                    plain_text = " ".join(w.split("_")[0] for w in response_text.split())
                    print(plain_text)
